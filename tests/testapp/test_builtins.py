...
from gargoyle.builtins import (
    ActiveTimezoneTodayConditionSet, AppTodayConditionSet, ConditionSet, HostConditionSet,
...
class UserConditionSetTests(TestCase):
    ...
    def test_user_is_superuser(self):
        conditions = self._create_contition('is_superuser', [('i', True)])
    ...
    def test_user_date_joined_after(self):
        conditions = self._create_contition('date_joined', [('i', self.date_str)])
    ...
    def test_user_in_percent_range(self):
        conditions = self._create_contition('percent', [('i', '0-50')])
    ...
    def test_user_is_anonymous(self):
        conditions = self._create_contition('is_anonymous', [('i', True)])
        user = AnonymousUser()
        assert self.condition_set.is_active(user, conditions) is True

    def test_user_is_not_anonymous(self):
        conditions = self._create_contition('is_anonymous', [('i', False)])
        user = AnonymousUser()
        assert not self.condition_set.is_active(user, conditions)
